...
with lib; let
  ...
  # ponytail: workaround for nixpkgs#546204 (blank Firefox chrome text);
  # drop once the wrapper re-adds gsettings schemas to XDG_DATA_DIRS
  gsettingsSchemas = concatMapStringsSep ":" (p: "${p}/share/gsettings-schemas/${p.name}") [
        ...
        programs.firefox = {
          ...
          .overrideAttrs (old: {
              buildCommand =
                old.buildCommand
                + ''
                  wrapProgram $out/bin/firefox --suffix XDG_DATA_DIRS : "${gsettingsSchemas}"
                '';
            });
          profiles.default = {
            id = 0;
