...
with lib; let
  ...
  # riderVersion = "${year}.${version}";
  pluginList = with inputs.nix-jetbrains-plugins.plugins."${pkgs.stdenv.hostPlatform.system}".rider."${year}.${version}"; [
    IdeaVIM
    Mermaid
  ];
in {
  ...
  config = mkIf cfg.enable {
    ...
    # xdg.configFile = {
    #   "JetBrains/Rider${riderVersion}/rider64.vmoptions" = ''
    #     # custom GoLand VM options (expand/override 'bin/rider.vmoptions')
    #
    #     -Dawt.toolkit.name=WLToolkit -Xmx4096m
    #   '';
    # };
    #
