{
  ...
  hyprlock-music = pkgs.writeShellScriptBin "hyprlock-music.sh" ''
    ...
    select_player() {
      if have ${pkgs.playerctl}/bin/playerctl && ${pkgs.playerctl}/bin/playerctl -p "$PREFERRED_PLAYERS" status >/dev/null 2>&1; then
    ...
    get_metadata() {
      ...
      if [[ -n "$player" ]]; then
        ${pkgs.playerctl}/bin/playerctl -p "$player" metadata --format "$fmt" 2>/dev/null || true
      else
        ${pkgs.playerctl}/bin/playerctl metadata --format "$fmt" 2>/dev/null || true
      fi
    }
    ...
    get_status() {
      ...
      if [[ -n "$player" ]]; then
        ${pkgs.playerctl}/bin/playerctl -p "$player" status 2>/dev/null || true
      else
        ${pkgs.playerctl}/bin/playerctl status 2>/dev/null || true
      fi
    }
    ...
    get_current_position() {
      ...
      if [[ -n "$player" ]]; then
        us="$(${pkgs.playerctl}/bin/playerctl -p "$player" position 2>/dev/null | ${pkgs.gawk}/bin/awk '{print int($1 * 1000000)}')" || true
      else
        us="$(${pkgs.playerctl}/bin/playerctl position 2>/dev/null | ${pkgs.gawk}/bin/awk '{print int($1 * 1000000)}')" || true
      fi
    ...
    calculate_progress_percent() {
      ...
      if [[ -n "$player" ]]; then
        pos_us="$(${pkgs.playerctl}/bin/playerctl -p "$player" position 2>/dev/null | ${pkgs.gawk}/bin/awk '{print int($1 * 1000000)}')" || true
        length_us="$(${pkgs.playerctl}/bin/playerctl -p "$player" metadata mpris:length 2>/dev/null)" || true
      else
        pos_us="$(${pkgs.playerctl}/bin/playerctl position 2>/dev/null | ${pkgs.gawk}/bin/awk '{print int($1 * 1000000)}')" || true
        length_us="$(${pkgs.playerctl}/bin/playerctl metadata mpris:length 2>/dev/null)" || true
      fi
    ...
    download_to_cache() {
      local url="$1"
      local filename
      filename="$(printf '%s' "$url" | ${pkgs.coreutils}/bin/sha256sum | ${pkgs.gawk}/bin/awk '{print $1}').img"
      local output="$CACHE_DIR/$filename"

      if [[ ! -s "$output" ]]; then
        have ${pkgs.curl}/bin/curl && ${pkgs.curl}/bin/curl -fsSL --max-time 5 "$url" -o "$output" || true
      fi
    ...
    create_square_cover() {
      local input="$1"
      local basename
      basename="$(${pkgs.coreutils}/bin/basename "$input")"
      ...
      if have ${pkgs.imagemagick}/bin/convert; then
        ${pkgs.imagemagick}/bin/convert "$input" -auto-orient -gravity center \
    ...
    get_status_icon() {
      case "$(get_status | ${pkgs.coreutils}/bin/tr '[:upper:]' '[:lower:]')" in
      playing)
        printf '󰏤'
        ;;
      paused)
    ...
    get_active_player() {
      ...
      if [[ -n "$player" ]]; then
        active_player="$(${pkgs.playerctl}/bin/playerctl -p "$player" -l 2>/dev/null | ${pkgs.coreutils}/bin/head -n1)"
      else
        active_player="$(${pkgs.playerctl}/bin/playerctl -l 2>/dev/null | ${pkgs.coreutils}/bin/head -n1)"
      fi
    ...
    --help | *)
      cat <<EOF
    Usage: $(${pkgs.coreutils}/bin/basename "$0") [OPTION]
    ...
    Examples:
      $(${pkgs.coreutils}/bin/basename "$0") --title
      $(${pkgs.coreutils}/bin/basename "$0") --progress-bar
...
in {
  config = lib.mkIf config.programs.hyprlock.enable {
    ...
    home.file.".config/hypr/scripts/hyprlock-music.sh" = {
      source = "${hyprlock-music}/bin/hyprlock-music.sh";
      executable = true;
    };

    # Create cache directory for album art
    home.file.".cache/hyprlock-art/.keep".text = "";
  };
}
