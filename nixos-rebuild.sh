...
if [ -x "/nix/store/wckka8fxv4h5hp74cbkhaw3fw7kbvcs1-bash-5.2p26/bin/bash" ]; then export SHELL="/nix/store/wckka8fxv4h5hp74cbkhaw3fw7kbvcs1-bash-5.2p26/bin/bash"; fi;

set -e
set -o pipefail
shopt -s inherit_errexit

export PATH=/nix/store/xfm4mg874w5n39zbqx24yiw7hmka94n7-coreutils-9.5/bin:/nix/store/h2gq8hngnsnwphzpq7992cip77lwrphm-gnused-4.9/bin:/nix/store/8p75w2cs6lgzakvz6q9xns7j876d0i8m-gnugrep-3.11/bin:/nix/store/c0x5axsm1lz5w89n81xfqqx83l0x4jjj-jq-1.7.1-bin/bin:/nix/store/2wvb4326f069mz8zan43yx6nak6lsjqk-util-linux-2.39.4-bin/bin:$PATH
...
if [ -z "$_NIXOS_REBUILD_REEXEC" ]; then
    export PATH=/nix/store/wxp968584yl2ygfm3kwhl712wc2y6r5k-nix-2.18.5/bin:$PATH
fi
...
prebuiltNix() {
    ...
    if [ "$machine" = x86_64 ]; then
        echo /nix/store/4rs65bbl7x7ba8xi65x3j9zfc219vp8j-nix-2.18.5
    elif [[ "$machine" =~ i.86 ]]; then
        echo /nix/store/qfzjbq958jnscjl1k8wr6gmpi5876dw6-nix-2.18.5
    elif [[ "$machine" = aarch64 ]]; then
        echo /nix/store/ns0ir4zmhn4bj55h3vvjcx1y3h5f6gy5-nix-2.18.5
    else
